...
@section('fail_chanel')
    <a href="{{route('all_prestations')}}"> <b> @lang('lang.prestations') </b></a>
@endsection
...
@section('content')
            ...
            <div class="x_panel">
                ...
                <div class="x_content">
                    <div class="card-box table-responsive">
                            ...
                            <form id="demo-form2"  method="POST" data-parsley-validate class="form-horizontal form-label-left">
                                ...
                                <div class="item ml-0">
                                    <div class="form-group col-xs-4">
                                        <label for="middle-name" class="col-form-label"> @lang('lang.date_start') : </label>
                                    ...
                                    <div class="form-group col-xs-4">
                                        <label for="middle-name" class="col-form-label"> @lang('lang.date_end') : </label>
                        ...
                        <table class="table" id="all_prestation">
                            <thead>
                                <tr>
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                        ...
                                        <a class="dropdown-item" href="#">
                                            <input type="radio" value="price" class="eltnav"
                                            onchange="return  group_by_technician();" name="group_by">
                                              @lang('lang.techniciens')
                                        </a>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @can('modify_prestation')
                                                    <a  href="{{route('modify_prestation_page', ['id' => $prestation->id])}}" class="btn btn-warning btn-sm" title="@lang('lang.modify')">  <i class="fa fa-pencil"></i> </a>
                                                @endcan
    ...
    <script>
        ...
        $("#search").click(function(){
            var url = "{{route('all_prestation_search')}}"
        ...
        $("#export").click(function(){
            var url = "{{route('service_export_excel')}}"
        ...
        function group_by_technician(){
            ...
            var ids_techniciens = [];
             @foreach ($all_prestations as $prestation)
                ...
                if (present_amount > 0) {
                    //alert("OK");
                    var present_technicien = Number("{{$prestation->Technicien->id}}");
                    ids_techniciens.push(present_technicien);
                    //alert(present_service);
            ...
            @foreach ($all_users as $user)
                ...
                var montant = 0;
                var iduser = "{{$user->id}}";
                var idus = Number("{{$user->id}}");

                if(ids_techniciens.includes(idus)){
                    table_perso += "<tr> <td>  <em style=\"font-style: italic;\">@lang('lang.technicien_ongl') </em> <b class='col-form-label'> {{strtoupper($user->name)}} {{ucfirst($user->surname)}}  </b> </td> <td>  </td> <td>  </td> <td> </td> <td>  </td> <td> </td> <td> </td><td> </td> </tr>";

                    @foreach ($all_prestations as $prestation)
                        var idcustumerPres = "{{$prestation->user_id}}";
                        if(iduser == idcustumerPres){
                            table_perso += "<tr> <td> {{date(App\Http\Controllers\user_function::date_format_value(), strtotime($prestation->date_prestation))}} </td> <td> {{$prestation->code}} </td> <td> {{$prestation->Technicien->name}} {{$prestation->Technicien->surname}} </td> <td> {{$prestation->Custumer->name}} {{$prestation->Custumer->surname}} </td> <td> {{$prestation->Service->name}} </td> <td> {{$prestation->price}} </td> <td> @can('modify_prestation') <a  href=\"{{route('modify_prestation_page', ['id' => $prestation->id])}}\" class=\"btn btn-warning btn-sm\" title=\"@lang('lang.modify')\">  <i class=\"fa fa-pencil\"></i> </a> @endcan @can('delete_prestation') <a href=\"#\" onclick=\" return valide_delete('{{$prestation->code}}', '{{$prestation->id}}');\" class=\"btn btn-danger btn-sm\" title=\"@lang('lang.delete')\">  <i class=\"fa fa-trash\"></i> </a> @endcan </td> <td> </td> </tr>" ;
                            montant += Number("{{$prestation->price}}") ;
